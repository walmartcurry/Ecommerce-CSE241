...
public class Order {
    ...
    private double discount;
    public static Map<String, PromoCode> promoCodeRepo = new HashMap<>();
    static {
...
public String getShippingAddress() {
    ...
    public void finalizeOrder() {
        if (orderStatus == OrderStatus.PAID) {
            ...
            for(CustomerProduct removeProd : myCart.cartItems){
                if(removeProd.getSupplierProd().getStock() <= 0){
                    remove.add(removeProd.getName());
                }
            }
            for(Customer cust : Database.users){
                ArrayList <CustomerProduct> removed = new ArrayList<CustomerProduct>();
                for(CustomerProduct prod : cust.getOrder().getMyCart().cartItems){
                    if(remove.contains(prod.getName())){
                        removed.add(prod);
                    }
                }
               cust.getOrder().getMyCart().cartItems.removeAll(removed);
            }
